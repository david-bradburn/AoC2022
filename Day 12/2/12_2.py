...
class Map():
	...
	def __init__(self, rawInput) -> None:
		...
		self.potentialstartPositions = []

		self.walkingDistances = []

		
		self.processInput()

		for potentialStart in self.potentialstartPositions:
			self.coordQueue = [potentialStart]

			self.distanceMap = np.ones_like(self.map, dtype=int)
			self.distanceMap *= BIGNUMBER
			self.distanceMap[potentialStart[1]][potentialStart[0]] = 0
		...
		print("--------------------------------")
		print(min(self.walkingDistances))
		...
		for rowIndex, row in enumerate(tempInput):
			for coloumnIndex, value in enumerate(row):
				if value == 'S':
					...
					self.potentialstartPositions.append((coloumnIndex, rowIndex))
	...
	def findSmallestdistance(self):
		...
		self.walkingDistances.append(self.distanceMap[self.endPosition[1]][self.endPosition[0]])
